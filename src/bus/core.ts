...
export class BusCoreImpl implements BusCore {
  ...
  private markAgentInitializing(agent_id: string): void {
    ...
    const timer = setTimeout(() => {
      ...
      this.markAgentReady(agent_id, true);
   ...
   *  `viaBackstop` = the flush is timer-driven (no `replay_done`), so the session
   *  may still be mid-(re)init and swallow the keystroke; verify a turn actually
   *  starts and re-deliver once if not. The real `replay_done` path is
   *  known-ready and needs no verification. */
  private markAgentReady(agent_id: string, viaBackstop = false): void {
