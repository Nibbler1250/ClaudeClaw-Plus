...
export class BusCoreImpl implements BusCore {
  ...
  private markAgentReady(agent_id: string, session_id?: string, viaBackstop = false): void {
    ...
    if (staleGeneration) {
      // A marker for a generation OLDER than the live one: the tailer being
      // replaced finishing its last read after the replacement published. It
      // must not move the live pointer back, clear the replacement's init hold,
      // flush the queue into a session that has not said it is ready, nor
      // release a compaction hold — a no-op, logged so a rotation storm shows.
      console.warn(
        `[bus] ignoring stale replay_done for agent=${agent_id} session=${session_id} (live=${this.agentLiveSession.get(agent_id)})`,
      );
      return;
    }
