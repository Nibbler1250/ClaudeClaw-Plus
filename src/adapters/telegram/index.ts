...
export class TelegramAdapter {
  ...
  private async handleResponseText(agentId: string, event: BusEvent): Promise<void> {
    ...
    // Receipt: the first assistant output (progress OR final) for this turn
    // proves the turn actually started generating. Stamp it so a `timeout`
    // receipt WITH `turn_started_at` reads as "turn generated but reply lost"
    // and one WITHOUT reads as "turn never started (prompt ingested, inference
    // never fired)" — the discriminator the wedge dossiers lacked (#233).
    this.stampTurnStarted(key);
  ...
  /** Stamp `turn_started_at` on the open receipt the first time any assistant
   *  output is seen for this turn. Idempotent (first stamp wins); a no-op when
   *  no receipt is open (unprompted cron/heartbeat replies). Purely additive —
   *  never touches receipt state or delivery flow. */
  /** (Re)arm the INACTIVITY timeout for an open receipt. The receipt closes as
   *  `timeout` only after `receiptTimeoutMs` of NO observed turn activity — not
   *  a fixed wall-clock budget from prompt arrival. Every assistant output
   *  (progress frame, tool call, text) rearms it via stampTurnStarted, so a
   *  long-but-actively-working turn is never falsely closed as a no-reply
   *  timeout (the failure that let the watchdog restart a healthy agent 14s
   *  before its reply landed, 2026-06-09). Only a genuinely silent turn — deaf
   *  agent / model hang — lets it fire. */
  ...
  private stampTurnStarted(key: string): void {
    const entry = this.openReceipts.get(key);
    if (!entry) return;
    // Observed turn activity -> rearm the inactivity timeout so a long, actively
    // working turn is never closed as a false no-reply timeout. Purely additive
    // to the existing turn_started_at stamp.
    this.armReceiptTimeout(key);
    const now = new Date().toISOString();
    const firstStamp = !entry.receipt.record.notes?.turn_started_at;
    entry.receipt.patch({
      notes: {
        ...entry.receipt.record.notes,
        ...(firstStamp ? { turn_started_at: now } : {}),
