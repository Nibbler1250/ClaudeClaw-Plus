...
const HARNESSES = {
  ...
  "web-research": {
    interpreter: PYTHON_PATH,
    interpreterArgs: [],
    script: "/home/simon/agent/scripts/voice-web-research-harness.py",
    cwd: "/home/simon/agent",
    requiredFields: [] as const, // query reused from description
  },
} as const;
...
function buildArgs(p: SpawnPayload): string[] {
  ...
  } else if (p.harness === "web-research") {
    args.push(
      "--task-id",
      p.task_id,
      "--query",
...
export async function handleSpawnHarness(req: Request, settings: Settings): Promise<Response> {
  // Auth
  if (!settings.apiToken || !checkToken(req, settings.apiToken)) {
  ...
  try {
    ...
    return json(
      { ok: false, error: `spawn failed: ${e instanceof Error ? e.message : String(e)}` },
      500,
    );
  }
