...
import { serveTunerOverMcp } from "../../wisecron/serve.js";
...
describe("McpTelemetryProvider over a real MCP transport", () => {
  it("reads capabilities + query through Client/Server (InMemory)", async () => {
    const audit = new AuditLog(join(tmpDir, "outcome-audit.jsonl"));
    const [clientT, serverT] = InMemoryTransport.createLinkedPair();
    await startMcpServer({ telemetry: new StubHostProvider(), audit, transport: serverT });

    const client = new Client({ name: "test", version: "0.0.0" }, { capabilities: {} });
    await client.connect(clientT);

    const provider = new McpTelemetryProvider(mcpClientToolCaller(client));
    const conn = await provider.connect();
    expect(conn.ok).toBe(true);

    expect(provider.capabilities().some((c) => c.stream === "cron_run" && c.available)).toBe(true);

    const samples = await provider.query("cron_run", RANGE);
    expect(samples).toHaveLength(1);
    expect(samples[0]!.ts instanceof Date).toBe(true);
    expect(samples[0]!.value).toBe(0);

    // Served query was recorded in the tamper-evident chain, chain verifies.
    const queries = audit.all().filter((r) => r.event === "telemetry_query");
    expect(queries).toHaveLength(1);
    expect(queries[0]!.detail).toMatchObject({ stream: "cron_run", sample_count: 1 });
...
describe("serveTunerOverMcp — end-to-end telemetry via MCP", () => {
  it("wires subjects to consume telemetry through MCP; activation + chain audited", async () => {
    ...
    const served = await serveTunerOverMcp(registry, settings, {
      hostProvider: new StubHostProvider(),
      audit,
      runHealthChecks: false,
    });

    // The tuner sees telemetry only through the MCP-backed provider.
    expect(served.mcpProvider).toBeInstanceOf(McpTelemetryProvider);
    expect(served.mcpProvider.isConnected()).toBe(true);
    expect(
      served.mcpProvider.capabilities().some((c) => c.stream === "cron_run" && c.available),
    ).toBe(true);

    // Activation gate ran against MCP-served capabilities and was audited.
    const events = audit.all().map((r) => r.event);
    expect(events.some((e) => e === "fitness_active" || e === "fitness_inactive")).toBe(true);
    expect(audit.verifyChain().ok).toBe(true);
  });
});
