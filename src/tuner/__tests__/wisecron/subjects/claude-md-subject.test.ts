...
describe("ClaudeMdSubject — apply / revert", () => {
  it("apply writes .bak before content replacement", async () => {
    ...
    const proposal: Proposal = {
      ...
      signature: "sig",
      alternatives: [
        { id: "fix-imports", label: "l", tradeoff: "", diff_or_content: "# new content\n" },
      ],
    };
  ...
  it("revert restores prior content (hash compare via .bak)", async () => {
    ...
    const inverse: Patch = {
      target_path: path,
      kind: "patch",
      applied_content: "unused-because-bak-wins",
    };
