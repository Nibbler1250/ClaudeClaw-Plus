...
function journalLine(opts: { unit: string; ts: Date; exit?: number; message?: string }): string {
    ...
    const obs: Observation[] = [
      {
        ...
        metadata: {
          subject: "cron",
          unit: "wisecron-bar.service",
          error_rate: 0,
          stale: true,
          command: "/usr/bin/bar.sh",
        },
      },
    ];
    const clusters = await subject.detectProblems(obs);
