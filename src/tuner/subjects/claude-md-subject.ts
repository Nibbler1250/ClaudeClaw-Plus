...
export class ClaudeMdSubject extends BaseSubject implements RevertibleSubject {
  ...
  async collectObservations(_since: Date): Promise<Observation[]> {
    ...
    for (const root of this.projectRoots) {
      ...
      for (const file of files) {
        ...
        const staleSections = ageDays > STALE_DAYS ? countSections(content) : 0;
  ...
  async proposeChange(cluster: Cluster): Promise<UnsignedProposal> {
    ...
    const brokenImports =
      ((firstObs.metadata as Record<string, unknown>).broken_imports as string[]) ?? [];
    ...
    return {
      ...
      alternatives: [
        {
          id: "fix-imports",
          label: "Remove broken @-imports",
          diff_or_content: fixed,
          tradeoff: "Quickest fix; consolidation deferred.",
        },
        {
          id: "consolidate",
          label: "Consolidate nested CLAUDE.md content",
          diff_or_content: fixed,
          tradeoff: "Single-source clarity; bigger diff.",
        },
        {
          id: "trim",
          label: "Trim stale sections",
          diff_or_content: trimmed,
          tradeoff: "Smaller file; some context lost.",
        },
      ],
