...
The HTTP gateway (PR #42) gave the daemon a common ingress with HMAC + 15-minute replay
window. `mcp-proxy` extends it: a single daemon-internal plugin that maintains long-lived
stdio connections to configured MCP servers and re-exposes their tools through the gateway.
